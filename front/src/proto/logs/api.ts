...
export const AddLogLinesRequest: MessageFns<AddLogLinesRequest> = {
  encode(message: AddLogLinesRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.lines) {
      LogLine.encode(v!, writer.uint32(10).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): AddLogLinesRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const SearchLogLinesRequest: MessageFns<SearchLogLinesRequest> = {
  encode(message: SearchLogLinesRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.exploit !== "") {
      writer.uint32(10).string(message.exploit);
    }
    if (!message.version.equals(Long.ZERO)) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): SearchLogLinesRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const SearchLogLinesResponse: MessageFns<SearchLogLinesResponse> = {
  encode(message: SearchLogLinesResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.lines) {
      LogLine.encode(v!, writer.uint32(10).fork()).join();
    }
    if (message.lastToken !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): SearchLogLinesResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
